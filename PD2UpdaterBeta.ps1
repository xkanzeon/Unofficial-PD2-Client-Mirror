...
function Receive-Google-Bucket {
    ...
    if ($response.StatusCode -eq '200') {
        ...
        if (!(Test-Path "$($pwd)/Beta/")) {
            New-Item -ItemType Directory -Force -Path "$($pwd)/Beta" | Out-Null
        }
        ...
        foreach ($file in $filelist) {
            try {
                ...
                if (!(Test-Path "$($pwd)/Beta/$(Split-Path $($file) -Parent)")) {
                    New-Item -ItemType Directory -Path "$($pwd)/Beta/$(Split-Path $($file) -Parent)" -Force | Out-Null
    ...
    if ($response.StatusCode -eq '200') {
        ...
        $filelist.GetEnumerator().ForEach({
            try {
                ...
                if (Test-Path "$($pwd)/Beta/$($_.Key)") {
                    $current = Get-FileHash -Algorithm MD5 "$($pwd)/Beta/$($_.Key)"
                    if ($current.Hash.ToLower() -eq $_.Value.ToLower()) {
                        Write-Host "[$('{0:d2}' -f $filecount)/$($filelist.Count)] $($_.Key) already updated. Skipping..."
                        Copy-Item -Path "$($pwd)/Beta/$($_.Key)" -Destination "$($pwd)/$($_.Key)"
                        $filecount += 1
                        return
                    }
                }
                ...
                Start-BitsTransfer "$($newclient)/$($_.Key)" "$($pwd)/Beta/$($_.Key)"
                Copy-Item -Path "$($pwd)/Beta/$($_.Key)" -Destination "$($pwd)/$($_.Key)"
